...
{
    [Serializable]
    public abstract class CategorizableIdentifiableAppalachiaObject : IdentifiableAppalachiaObject,
                                                                      ICategorizable
    {
...
#pragma warning restore 0649
        ...
        private void SelectCategory()
        {
            UnityEditor.Selection.objects = GetAllOfType(
                    GetType(),
                    i => ((CategorizableIdentifiableAppalachiaObject) i).Category == Category
                )
               .ToArray();
        }
        ...
        private void SelectUncategorized()
        {
            UnityEditor.Selection.objects = GetAllOfType(
                    GetType(),
                    i => string.IsNullOrWhiteSpace(((CategorizableIdentifiableAppalachiaObject) i).Category)
                )
               .ToArray();
        }
