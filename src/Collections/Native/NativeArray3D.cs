...
namespace Appalachia.Core.Collections.Native
{
        ...
        where T : struct
    {
        ...
        public int3 Length
        {
            get => new(_length0, _length1, _length2);
            set
            {
                CheckWriteAccess();
        ...
        private static void Copy(NativeArray3D<T> src, NativeArray3D<T> dest)
        {
            ...
            if ((src.Length0 == dest.Length0) &&
                ...
                (src.Length2 == dest.Length2))
            {
                providedLength0 = dest.Length0;
                providedLength1 = dest.Length1;
                providedLength2 = dest.Length2;
            }
            else if ((src.Capacity0 == dest.Capacity0) &&
                     (src.Capacity1 == dest.Capacity1) &&
                     (src.Capacity2 == dest.Capacity2))
            {
        ...
        private static void Copy(T[,,] src, NativeArray3D<T> dest)
        {
            ...
            if (((providedLength0 == dest.Length0) &&
                 (providedLength1 == dest.Length1) &&
                 (providedLength2 == dest.Length2)) ||
                ((providedLength0 == dest.Capacity0) &&
                 (providedLength1 == dest.Capacity1) &&
        ...
        private static void Copy(NativeArray3D<T> src, T[,,] dest)
        {
            ...
            if (((providedLength0 == src.Length0) &&
                 (providedLength1 == src.Length1) &&
                 (providedLength2 == src.Length2)) ||
                ((providedLength0 == src.Capacity0) &&
                 (providedLength1 == src.Capacity1) &&
                 (providedLength2 == src.Capacity2)))
            {
                for (var index0 = 0; index0 < providedLength0; ++index0)
                {
                    for (var index1 = 0; index1 < providedLength1; ++index1)
