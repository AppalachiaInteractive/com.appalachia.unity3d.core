...
{
	/// <summary>
	///     A pointer to an float2 stored in native (i.e. unmanaged) memory
	/// </summary>
	[NativeContainer]
    [NativeContainerSupportsDeallocateOnJobCompletion]
    [DebuggerTypeProxy(typeof(NativeFloat2PtrDebugView))]
    ...
    public unsafe struct NativeFloat2Ptr : IDisposable
    {
        ...
        public struct Parallel
        {
            ...
            internal readonly float2* m_Buffer;
	        ...
	        /// </param>
	        internal Parallel(float2* value, AtomicSafetyHandle safety)
            {
                m_Buffer = value;
                m_Safety = safety;
            }
			...
			/// Pointer to the value
			/// </param>
			internal Parallel(float2* value)
			{
				m_Buffer = value;
			}
#endif
	        ...
	        /// </returns>
	        [WriteAccessRequired]
            public void Subtract(float2 value)
            {
                CheckWriteAccess();
	        ...
	        /// </returns>
	        [WriteAccessRequired]
            public void Multiply(float2 value)
            {
                CheckWriteAccess();
	        ...
	        /// </returns>
	        [WriteAccessRequired]
            public void Divide(float2 value)
            {
                CheckWriteAccess();
	    ...
	    public NativeFloat2Ptr(Allocator allocator, float2 initialValue = default)
        {
            ...
            m_Buffer = (float2*) UnsafeUtility.Malloc(sizeof(float2), UnsafeUtility.AlignOf<float2>(), allocator);
