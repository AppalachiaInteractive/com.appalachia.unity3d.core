...
{
	/// <summary>
	///     A pointer to an float4 stored in native (i.e. unmanaged) memory
	/// </summary>
	[NativeContainer]
    [NativeContainerSupportsDeallocateOnJobCompletion]
    [DebuggerTypeProxy(typeof(NativeFloat4PtrDebugView))]
    [DebuggerDisplay("Value = {" + nameof(Value) + "}")]
    [StructLayout(LayoutKind.Sequential)]
    public unsafe struct NativeFloat4Ptr : IDisposable
    {
	    /// <summary>
        ...
        public struct Parallel
        {
            ...
            internal readonly float4* m_Buffer;
	        ...
	        /// </param>
	        internal Parallel(float4* value, AtomicSafetyHandle safety)
            {
                m_Buffer = value;
                m_Safety = safety;
            }
			...
			/// Pointer to the value
			/// </param>
			internal Parallel(float4* value)
			{
				m_Buffer = value;
			}
#endif
	        ...
	        /// </returns>
	        [WriteAccessRequired]
            public void Subtract(float4 value)
            {
                CheckWriteAccess();
	        ...
	        /// </returns>
	        [WriteAccessRequired]
            public void Multiply(float4 value)
            {
                CheckWriteAccess();
	        ...
	        /// </returns>
	        [WriteAccessRequired]
            public void Divide(float4 value)
            {
                CheckWriteAccess();
	    ...
	    /// </param>
	    public NativeFloat4Ptr(Allocator allocator, float4 initialValue = default)
        {
            ...
            m_Buffer = (float4*) UnsafeUtility.Malloc(sizeof(float4), UnsafeUtility.AlignOf<float4>(), allocator);
