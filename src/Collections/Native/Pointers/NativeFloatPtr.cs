...
{
	/// <summary>
	///     A pointer to an float stored in native (i.e. unmanaged) memory
	/// </summary>
	[NativeContainer]
    [NativeContainerSupportsDeallocateOnJobCompletion]
    [DebuggerTypeProxy(typeof(NativeFloatPtrDebugView))]
    [DebuggerDisplay("Value = {" + nameof(Value) + "}")]
    [StructLayout(LayoutKind.Sequential)]
    public unsafe struct NativeFloatPtr : IDisposable
    {
	    /// <summary>
        ...
        public struct Parallel
        {
            ...
            internal readonly float* m_Buffer;
	    ...
	    /// </param>
	    public NativeFloatPtr(Allocator allocator, float initialValue = 0)
        {
            ...
            m_Buffer = (float*) UnsafeUtility.Malloc(sizeof(float), UnsafeUtility.AlignOf<float>(), allocator);
