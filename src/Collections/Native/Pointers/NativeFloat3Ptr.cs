...
{
	/// <summary>
	///     A pointer to an float3 stored in native (i.e. unmanaged) memory
	/// </summary>
	[NativeContainer]
    ...
    public unsafe struct NativeFloat3Ptr : IDisposable
    {
        ...
        public struct Parallel
        {
            ...
            internal readonly float3* m_Buffer;
	        ...
	        /// </param>
	        internal Parallel(float3* value, AtomicSafetyHandle safety)
            {
                m_Buffer = value;
                m_Safety = safety;
            }
			...
			/// Pointer to the value
			/// </param>
			internal Parallel(float3* value)
			{
				m_Buffer = value;
			}
#endif
	        ...
	        /// </returns>
	        [WriteAccessRequired]
            public void Subtract(float3 value)
            {
                CheckWriteAccess();
	        ...
	        /// </returns>
	        [WriteAccessRequired]
            public void Multiply(float3 value)
            {
                CheckWriteAccess();
	        ...
	        /// </returns>
	        [WriteAccessRequired]
            public void Divide(float3 value)
            {
                CheckWriteAccess();
	    ...
	    /// </param>
	    public NativeFloat3Ptr(Allocator allocator, float3 initialValue = default)
        {
            ...
            m_Buffer = (float3*) UnsafeUtility.Malloc(sizeof(float3), UnsafeUtility.AlignOf<float3>(), allocator);
