...
namespace Appalachia.Core.Collections.Native
{
    ...
    public unsafe struct NativeChunkedList<T> : IEnumerable<T>, IDisposable
...
#endif
    {
        ...
        public struct ChunksEnumerator : IEnumerator<IEnumerable<T>>
        {
	        ...
	        public ChunkEnumerable Current
            {
                get
                {
                    ...
                    // Just one chunk
                    var state = m_List.m_State;
                    var chunk = state->m_Chunks[m_Index].m_Values;
                    int startChunkIndex;
                    int endChunkIndex;
	        ...
	        ///     An enumerable for the current chunk
	        /// </value>
	        IEnumerable<T> IEnumerator<IEnumerable<T>>.Current => Current;
	    ...
	    /// </param>
	    public NativeChunkedList(int chunkLength, int capacity, Allocator allocator)
        {
	    ...
	    public int Capacity
        {
            ...
            set
            {
                ...
                var chunkLength = m_State->m_ChunkLength;
                var newCapacity = (((value + chunkLength) - 1) / chunkLength) * chunkLength;

                // Number of chunks has increased
                var numOldChunks = m_State->m_ChunksLength;
                var numNewChunks = newCapacity / chunkLength;
                if (numNewChunks > numOldChunks)
                {
                    // Need more chunks than we have available
                    var allocator = m_State->m_AllocatorLabel;
                    NativeChunkedListChunk* chunks;
                    var chunksCapacity = m_State->m_ChunksCapacity;
                    if (numNewChunks > chunksCapacity)
                    {
                        // Compute the new chunks capacity
                        // Try to double, but meet the minimum
                        var grown = chunksCapacity * 2;
                        var min = numNewChunks > 64 ? numNewChunks : 64;
                    ...
                    var size = chunkLength * UnsafeUtility.SizeOf<T>();
                    var align = UnsafeUtility.AlignOf<T>();
                    for (var i = numOldChunks; i < numNewChunks; ++i)
                    {
                        chunks[i].m_Values = UnsafeUtility.Malloc(size, align, allocator);
                        UnsafeUtility.MemClear(chunks[i].m_Values, size);
                    }
                ...
                else if (value < m_State->m_Length)
                {
                    ...
                    if (numNewChunks < numOldChunks)
                    {
                        ...
                        var allocator = m_State->m_AllocatorLabel;
                        for (var i = numNewChunks; i < numOldChunks; ++i)
                        {
                            UnsafeUtility.Free(m_State->m_Chunks[i].m_Values, allocator);
                        }
	    ...
	    ///     <see cref="Length" />.
	    /// </param>
	    public ChunksEnumerable GetChunksEnumerable(int startIndex, int endIndex)
        {
            CheckReadAccess();
	    ...
	    public T this[int index]
        {
            get
            {
                ...
                var chunkArrayIndex = index % chunkLength;
                var chunkArray = m_State->m_Chunks[chunkIndex].m_Values;
                return UnsafeUtility.ReadArrayElement<T>(chunkArray, chunkArrayIndex);
            }
            ...
            set
            {
                ...
                var chunkArrayIndex = index % chunkLength;
                var chunkArray = m_State->m_Chunks[chunkIndex].m_Values;
                UnsafeUtility.WriteArrayElement(chunkArray, chunkArrayIndex, value);
            }
        }
        ...
        public void AddRange(T[] array, int startIndex, int length)
        {
            ...
            var oldLength = m_State->m_Length;
            var newLength = oldLength + length;
            var capacity = m_State->m_Capacity;
	    ...
	    /// </returns>
	    public T[] ToArray(T[] array = null, int startIndex = 0, int destIndex = 0, int length = -1)
        {
            CheckReadAccess();
        ...
        public void Dispose()
        {
            ...
            for (var i = 0; i < m_State->m_ChunksLength; ++i)
            {
                UnsafeUtility.Free(m_State->m_Chunks[i].m_Values, m_State->m_AllocatorLabel);
            }
...
// No check is necessary because C# 7.3 uses `where T : unmanaged`
        ...
        private void RequireIndicesInBounds(int startIndex, int endIndex)
        {
            ...
            if (startIndex < 0)
            {
                throw new IndexOutOfRangeException("Invalid start index. It must be non-negative");
            }
