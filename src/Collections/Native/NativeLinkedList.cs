...
namespace Appalachia.Core.Collections.Native
{
    ...
    public unsafe struct NativeLinkedList<T> : IEnumerable<T>, IDisposable
...
#endif
    {
        ...
        public struct Enumerator : IEnumerator<T>, IEquatable<Enumerator>
        {
	        ...
	        /// </param>
	        internal Enumerator(NativeLinkedList<T> list, int index, int version)
            {
                m_Index = index;
                m_Version = version;
                m_List = list;
            ...
            /// </value>
            public NativeLinkedList<T> List => m_List;
            ...
            public Enumerator Next
            {
                get
                {
                    ...
                    if (m_Version == m_List.m_State->m_Version)
                    {
                        ...
                        return new Enumerator(m_List, m_List.m_State->m_HeadIndex, m_Version);
                    }
            ...
            public Enumerator Prev
            {
                get
                {
                    ...
                    if (m_Version == m_List.m_State->m_Version)
                    {
                        ...
                        return new Enumerator(m_List, m_List.m_State->m_TailIndex, m_Version);
                    }
...
#pragma warning restore RECS0025
	    ...
	    public NativeLinkedList(int capacity, Allocator allocator)
        {
            ...
            // operations
            m_State->m_AllocatorLabel = allocator;
	    ...
	    public NativeLinkedList(int capacity, int length, Allocator allocator)
        {
            ...
            // Insist on a minimum capacity
            var requiredCapacity = math.max(4, length);
	    ...
	    public Enumerator Tail
        {
            get
            {
                ...
                return new Enumerator(this, m_State->m_TailIndex, m_State->m_Version);
            }
        }
	    ...
	    public T this[int index]
        {
            get
            {
                ...
                return UnsafeUtility.ReadArrayElement<T>(m_State->m_Values, index);
            }
            ...
            set
            {
                ...
                UnsafeUtility.WriteArrayElement(m_State->m_Values, index, value);
            }
        }
        ...
        public Enumerator InsertAfter(Enumerator enumerator, T value)
        {
            ...
            var insertNextIndex = m_State->m_NextIndexes[enumerator.m_Index];
	    ...
	    /// </returns>
	    [WriteAccessRequired]
        public Enumerator InsertAfter(Enumerator enumerator, NativeLinkedList<T> list)
        {
            CheckReadAccess();
            CheckWriteAccess();
            RequireFullListSafetyCheckBounds();
            list.CheckReadAccess();
	    ...
	    /// </returns>
	    [WriteAccessRequired]
        public Enumerator InsertAfter(Enumerator enumerator, NativeArray<T> array)
        {
            CheckReadAccess();
            CheckWriteAccess();
            RequireFullListSafetyCheckBounds();
	    ...
	    /// </returns>
	    [WriteAccessRequired]
        public Enumerator InsertAfter(Enumerator enumerator, Enumerator start, Enumerator end)
        {
            CheckReadAccess();
            CheckWriteAccess();
            RequireFullListSafetyCheckBounds();
            ...
            var numInsertNodes = start.GetDistance(end) + 1;
        ...
        public Enumerator InsertAfter(Enumerator enumerator, NativeArray<T> array, int startIndex, int length)
        {
            ...
            if (!enumerator.IsValidFor(this))
            {
                ...
                // Copy to the end
                CopyToEnd(array, startIndex, length, out copiedHeadIndex, out copiedTailIndex);
	    ...
	    /// </returns>
	    [WriteAccessRequired]
        public Enumerator InsertAfter(Enumerator enumerator, T[] array, int startIndex, int length)
        {
            CheckReadAccess();
            CheckWriteAccess();
            RequireFullListSafetyCheckBounds();
        ...
        public Enumerator InsertBefore(Enumerator enumerator, T value)
        {
            ...
            var insertPrevIndex = m_State->m_PrevIndexes[enumerator.m_Index];
	    ...
	    /// </returns>
	    [WriteAccessRequired]
        public Enumerator InsertBefore(Enumerator enumerator, NativeLinkedList<T> list)
        {
            CheckReadAccess();
            CheckWriteAccess();
            RequireFullListSafetyCheckBounds();
	    ...
	    /// </returns>
	    [WriteAccessRequired]
        public Enumerator InsertBefore(Enumerator enumerator, NativeArray<T> array)
        {
            CheckReadAccess();
            CheckWriteAccess();
            RequireFullListSafetyCheckBounds();
	    ...
	    /// </returns>
	    [WriteAccessRequired]
        public Enumerator InsertBefore(Enumerator enumerator, Enumerator start, Enumerator end)
        {
            CheckReadAccess();
            CheckWriteAccess();
            RequireFullListSafetyCheckBounds();
	    ...
	    /// </returns>
	    [WriteAccessRequired]
        public Enumerator InsertBefore(Enumerator enumerator, T[] array, int startIndex, int length)
        {
            CheckReadAccess();
            CheckWriteAccess();
            RequireFullListSafetyCheckBounds();
        ...
        public Enumerator Remove(Enumerator enumerator)
        {
            ...
            else
            {
                ...
                if (enumerator.m_Index == m_State->m_HeadIndex)
                {
                    ...
                    var nextIndex = m_State->m_NextIndexes[enumerator.m_Index];
                ...
                else if (enumerator.m_Index == m_State->m_TailIndex)
                {
                    ...
                    var prevIndex = m_State->m_PrevIndexes[enumerator.m_Index];
                ...
                var lastIndex = m_State->m_Length - 1;
                if (enumerator.m_Index != lastIndex)
                {
                    // Copy the last node to where the removed node was
                    var lastNextIndex = m_State->m_NextIndexes[lastIndex];
                    var lastPrevIndex = m_State->m_PrevIndexes[lastIndex];
                    UnsafeUtility.WriteArrayElement(
                        m_State->m_Values,
                        enumerator.m_Index,
                        UnsafeUtility.ReadArrayElement<T>(m_State->m_Values, lastIndex)
                    );
        ...
        public void Swap(Enumerator a, Enumerator b)
        {
            ...
            if (a.IsValidFor(this) && b.IsValidFor(this) && (a.m_Index != b.m_Index))
            {
        ...
        public void SortNodeMemoryAddresses()
        {
            ...
            var endIndex = m_State->m_Length - 1;
        ...
        public void Dispose()
        {
            ...
            UnsafeUtility.Free(m_State, m_State->m_AllocatorLabel);
            m_State = null;
        }
	    ...
	    private void CopyToEnd(NativeLinkedList<T> list, out int copiedHeadIndex, out int copiedTailIndex)
        {
            ...
            for (var i = 0; i < list.m_State->m_Length; ++i)
            {
                m_State->m_NextIndexes[endIndex + i] = list.m_State->m_NextIndexes[i] + endIndex;
            }

            for (var i = 0; i < list.m_State->m_Length; ++i)
            {
                m_State->m_PrevIndexes[endIndex + i] = list.m_State->m_PrevIndexes[i] + endIndex;
            }
            ...
            out int copiedTailIndex)
        {
            ...
            do
            {
                ...
                UnsafeUtility.WriteArrayElement(
                    m_State->m_Values,
                    destIndex,
                    UnsafeUtility.ReadArrayElement<T>(start.m_List.m_State->m_Values, srcIndex)
                );
            ...
            for (var i = copiedHeadIndex; i < copiedTailIndex; ++i)
            {
                m_State->m_NextIndexes[i] = i + 1;
            }
            ...
            m_State->m_PrevIndexes[copiedHeadIndex] = -1;
            for (var i = copiedHeadIndex + 1; i <= copiedTailIndex; ++i)
            {
                m_State->m_PrevIndexes[i] = i - 1;
            }
        }
	    ...
	    ///     Index that the list's tail node was copied to
	    /// </param>
	    private void CopyToEnd(T[] array, out int copiedHeadIndex, out int copiedTailIndex)
        {
            // Compute the indices of the head and tail of the copied portion of
            // the list
            var endIndex = m_State->m_Length;
	    ...
	    private void EnsureCapacity(int numNewNodes)
        {
            if ((m_State->m_Length + numNewNodes) > m_State->m_Capacity)
            {
                ...
                var newCapacity = math.max(m_State->m_Length * 2, m_State->m_Length + numNewNodes);
...
// No check is necessary because C# 7.3 uses `where T : unmanaged`
        ...
        private void RequireNonNullManagedArray(T[] array)
        {
            ...
            if (array == null)
            {
                throw new ArgumentNullException(nameof(array), "The given array must not be null.");
            }
#endif
        }
	    ...
	    /// </param>
	    [Conditional("ENABLE_UNITY_COLLECTIONS_CHECKS")]
        private void RequireValidRangeWithinArray(NativeArray<T> array, int startIndex, int length)
        {
#if ENABLE_UNITY_COLLECTIONS_CHECKS
            if (length < 0)
            {
	    ...
	    /// </param>
	    [Conditional("ENABLE_UNITY_COLLECTIONS_CHECKS")]
        private void RequireValidRangeWithinArray(T[] array, int startIndex, int length)
        {
#if ENABLE_UNITY_COLLECTIONS_CHECKS
            if (length < 0)
            {
