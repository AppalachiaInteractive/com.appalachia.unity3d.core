...
{
    [Serializable]
    public abstract class AppaLookup4<TKey1, TKey2, TKey3, TKey4, TValue, TKey1List, TKey2List,
                                      TKey3List, TKey4List, TValueList, TSubSubNested, TSubNested,
                                      TNested, TSubSubNestedList, TSubNestedList, TNestedList> :
        ...
        where TSubNested : AppaLookup2<TKey3, TKey4, TValue, TKey3List, TKey4List, TValueList,
            TSubSubNested, TSubSubNestedList>, new()
        where TNested : AppaLookup3<TKey2, TKey3, TKey4, TValue, TKey2List, TKey3List, TKey4List,
            TValueList, TSubSubNested, TSubNested, TSubSubNestedList, TSubNestedList>, new()
...

    {
        private const string _PRF_PFX =
            nameof(AppaLookup4<TKey1, TKey2, TKey3, TKey4, TValue, TKey1List, TKey2List, TKey3List,
                TKey4List, TValueList, TSubSubNested, TSubNested, TNested, TSubSubNestedList,
                TSubNestedList, TNestedList>) +
            ".";
                ...
                if (base.TryGetValue(primary, out var sub1))
                {
                    if (sub1.TryGetValue(secondary, out var sub2))
                    {
                        if (sub2.TryGetValue(tertiary, out var sub3))
                        {
                            ...
                            value = sub3.FirstWithPreference_NoAlloc(
                                fallbackCheck,
                                out var foundFallback
                            );
