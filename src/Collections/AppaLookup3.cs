...
namespace Appalachia.Core.Collections
{
    ...
    public abstract class AppaLookup3<TKey1, TKey2, TKey3, TValue, TKey1List, TKey2List, TKey3List,
                                      TValueList, TSubNested, TNested, TSubNestedList,
                                      TNestedList> : AppaLookup<TKey1, TNested, TKey1List,
        TNestedList>
        ...
        where TNested : AppaLookup2<TKey2, TKey3, TValue, TKey2List, TKey3List, TValueList,
            TSubNested, TSubNestedList>, new()
        where TSubNestedList : AppaList<TSubNested>, new()
        where TNestedList : AppaList<TNested>, new()
    {
        private const string _PRF_PFX =
            nameof(AppaLookup3<TKey1, TKey2, TKey3, TValue, TKey1List, TKey2List, TKey3List,
            ...
            using (_PRF_TryGetValueWithFallback.Auto())
            {
                if (base.TryGetValue(primary, out var sub1))
                {
                    if (sub1.TryGetValue(secondary, out var sub2))
                    {
                        ...
                        value = sub2.FirstWithPreference_NoAlloc(
                            fallbackCheck,
