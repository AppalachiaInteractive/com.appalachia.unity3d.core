...
namespace Appalachia.Core.Math.Geometry
{
    public struct Barycentric
    {
        ...
        public Barycentric(Vector4 aV1, Vector4 aV2, Vector4 aV3, Vector4 aP)
        {
            ...
            var aLen = (a.x * a.x) + (a.y * a.y) + (a.z * a.z) + (a.w * a.w);
            var bLen = (b.x * b.x) + (b.y * b.y) + (b.z * b.z) + (b.w * b.w);
            var ab = (a.x * b.x) + (a.y * b.y) + (a.z * b.z) + (a.w * b.w);
            var ac = (a.x * c.x) + (a.y * c.y) + (a.z * c.z) + (a.w * c.w);
            var bc = (b.x * c.x) + (b.y * c.y) + (b.z * c.z) + (b.w * c.w);
            var d = (aLen * bLen) - (ab * ab);
            u = ((aLen * bc) - (ab * ac)) / d;
            v = ((bLen * ac) - (ab * bc)) / d;
            w = 1.0f - u - v;
