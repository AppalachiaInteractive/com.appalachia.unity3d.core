...
namespace Appalachia.Core.Extensions
{
    public static class ObjectInvokeExtensions
    {
        ...
        private static readonly object[] _xv = new object[0];
        ...
        public static TR InvokeReturn<T,TR>(this T o, string methodName)
        {
            using (_PRF_InvokeReturn.Auto())
            {
                ...
                using (_PRF_InvokeReturn_GetBestMethod.Auto())
                {
                    bestMethod = ReflectionCache.GetBestMethod(methods, typeof(TR), _xt);
                }

                using (_PRF_InvokeReturn_InvokeMethod.Auto())
                {
                    result = bestMethod.Invoke(o, _xv);
                }
        ...
        public static void InvokeDynamic(this Type t, string methodName)
        {
            using (_PRF_Invoke.Auto())
            {
                ...
                var method = ReflectionCache.GetBestMethod(methods, null, _xt).Invoke(t, _xv);
            }
        }
        ...
        public static TR InvokeReturn<TR>(this Type t, string methodName)
        {
            ...
            {
                ReflectionCache.PrepareMethodInfo(t, methodName, NONPUBLIC_STATIC, out var methods);
                return (TR) ReflectionCache.GetBestMethod(methods, typeof(TR), _xt).Invoke(t, _xv);
            }
        }
